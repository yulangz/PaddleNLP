...
template <typename T>
static std::vector<T> get_rand(int64_t cnt, T min, T max, int seed) {
    std::default_random_engine e(seed);
    std::uniform_real_distribution<float> rand_engine(min, max);
    std::vector<T> vec(cnt);
    for (size_t i = 0; i < vec.size(); i++) {
        vec[i] = static_cast<T>(rand_engine(e));
    }
    return vec;
}
template <typename T, size_t dim>
static xft::xftTensor<T, dim> create_test_tensor(
        api::Context* ctx,
        const std::array<int64_t, dim>& shape,
        float range_low = -1.0f,
        float range_high = 1.0f,
        int seed = 12345) {
    using CpuType = float16;
    std::vector<CpuType> vec_cpu = get_rand<CpuType>(
            std::accumulate(shape.begin(), shape.end(), 1, std::multiplies<int64_t>()),
            range_low,
            range_high,
            seed);
    return xft::weight_from_host<T, dim>(ctx, vec_cpu.data(), shape);
...
std::vector<paddle::Tensor> MlaAttn(
  ...
  using QType = typename XPUTypeTrait<float16>::Type;
  using CacheType = typename XPUTypeTrait<float16>::Type;
  typedef paddle::float16 qdata_t, cache_t;
//   std::cout << "!!!!! kernel !!!!!!!!!" << std::endl;
  ...
  auto q_xft = baidu::xpu::xft::xftTensor<QType, 2>(
      reinterpret_cast<QType*>(const_cast<paddle::float16*>(q.data<qdata_t>())),
      std::array<int64_t, 2>{q.shape()[0],
                             q.shape()[1]});
  auto k_xft = baidu::xpu::xft::xftTensor<QType, 2>(
      reinterpret_cast<QType*>(const_cast<paddle::float16*>(k.data<qdata_t>())),
      std::array<int64_t, 2>{k.shape()[0],
                             k.shape()[1]});
  auto v_xft = baidu::xpu::xft::xftTensor<QType, 2>(
      reinterpret_cast<QType*>(const_cast<paddle::float16*>(v.data<qdata_t>())),
      std::array<int64_t, 2>{v.shape()[0],
                             v.shape()[1]});                             
  // 初始化输入：k cache
  auto key_cache_xft = baidu::xpu::xft::xftTensor<CacheType, 4>(
  reinterpret_cast<CacheType*>(const_cast<paddle::float16*>(key_cache.data<cache_t>())),
  ...
  reinterpret_cast<CacheType*>(const_cast<paddle::float16*>(value_cache.data<cache_t>())),
  ...
  auto fmha_out_xft = baidu::xpu::xft::xftTensor<QType, 2>(
      reinterpret_cast<QType*>(const_cast<paddle::float16*>(fmha_out.data<qdata_t>())),
...
//   std::cout << std::endl;
  ...
  if(max_enc_len_this_time.data<int>()[0] > 0){
    ...
    int ret_cache = xftkernel::xft_reshape_cached_kv<QType, CacheType, int>(
            xpu_ctx->x_context(),
            k_xft.data(),
            key_cache_xft.data(),
            ...
            nullptr); // max_ptrs
    ret_cache = xftkernel::xft_reshape_cached_kv<QType, CacheType, int>(
            xpu_ctx->x_context(),
            v_xft.data(),
            value_cache_xft.data(),
            block_tables_xft.data(),
            context_len_vp,
            start_tokens_vp,
            valid_batch_vp,
            ...
            nullptr); // max_ptrs
    // fmha op
    using FMHA_Type = typename baidu::xpu::xft::FMHA_QFp16_KVFp16;
    ...
    // std::cout << "fmha kernel begin " <<std::endl;
    // int ret = fmha_op.forward(
    //         xpu_ctx->x_context(),
    //         qbuf_xft,
    //         k_xft,
    //         v_xft,
    ...
    //         &attn_param_.vllm_block_tables_);  
  ...
  if(max_dec_len_this_time.data<int>()[0] > 0){
    ...
    int ret_cache = xftkernel::xft_reshape_cached_kv<QType, CacheType, int>(
            xpu_ctx->x_context(),
            k_xft.data() + total_enc_len * dim_qk * num_head,
            key_cache_xft.data(),
            block_tables_xft.data(),
            kv_seq_lod_vp,
            context_len_vp,
    ...
    ret_cache = xftkernel::xft_reshape_cached_kv<QType, CacheType, int>(
            xpu_ctx->x_context(),
            v_xft.data() + total_enc_len * dim_v * num_head,
            value_cache_xft.data(),
            block_tables_xft.data(),
            kv_seq_lod_vp,
            context_len_vp,
            ...
            nullptr); // max_ptrs
    // paged attention op
    using PA_Type = typename baidu::xpu::xft::PA_QFp16_KVFp16;
    using TQ = typename PA_Type::Qtype;
    using TKV = typename PA_Type::KVtype;
    using TO = typename PA_Type::Otype;
    using TID = typename PA_Type::TIDtype;
    using TGEMM0 = float16;
    using TGEMM1 = float16;
    using TEW = float16;
    using TL = typename kl3_pa_TL_trait<TQ>::TL;
    auto pa_func = &baidu::xpu::xfa::paged_attention_xft<TQ, TKV, TO, TL, TGEMM0, TGEMM1, TEW, TID, false>;
    baidu::xpu::xft::PageAttnOperation<PA_Type> pa_op(page_param_, attn_param_);  
    // int ret = pa_op.forward(
    //         xpu_ctx->x_context(),
    //         q_xft,
    //         key_cache_xft,
    //         value_cache_xft,
    ...
    //         &fmha_out_xft,
    //         nullptr, // p_query_max,
    //         nullptr, // p_kcache_perhead_scale == nullptr ? nullptr : p_kcache_perhead_scale->data(),
    //         nullptr, // p_vcache_perhead_scale == nullptr ? nullptr : p_vcache_perhead_scale->data(),
    //         nullptr, // p_kcache_max,
    //         nullptr, // p_vcache_max,
    //         nullptr, // p_out_max
    //         nullptr, // p_shift,
    //         nullptr, // p_smooth,
    //         nullptr, // p_bias 
    //         nullptr, // use_cur_kv_ ? &kbuf : nullptr, 
    //         nullptr, //use_cur_kv_ ? &vbuf : nullptr,
