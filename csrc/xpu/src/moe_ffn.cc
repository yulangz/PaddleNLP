...
std::vector<paddle::Tensor> MoeFFNKernel(
  ...
  int inter_dim = ffn1_weight.shape()[1];
  ...
  auto ffn1_input = baidu::xpu::xft::xftTensor<XPUType, 2>(
      reinterpret_cast<XPUType*>(const_cast<paddle::float16*>(permute_input.data<data_t>())),
  ...
  auto ffn1_w_tensor = baidu::xpu::xft::xftMat<int8_t>(
      ...
      nullptr, 
      const_cast<float*>(ffn1_scale->data<float>()),  //这个地方GPU是float16，我们需要的是float，需要check
      std::array<int64_t, 2>{num_experts * inter_dim, hidden_size});
  ...
  int ret = baidu::xpu::xft::xft_moe_sort_fc_block<XPUType,
                                                   int8_t,
                                                   XPUType,
                                                   float,
                                                   int32_t,
      ...
      2,
      1);
  auto ffn2_input_paddle = paddle::full({expanded_active_expert_rows, inter_size / 2},
                                      -1,
                                      permute_input.type(),
                                      permute_input.place());
 ...
 ret = xftkernel::xft_fast_swiglu_add_mul_fusion<XPUType>(
      ...
      nullptr, 
      const_cast<float*>(ffn2_scale->data<float>()),  //这个地方GPU是float16，我们需要的是float，需要check
      std::array<int64_t, 2>{num_experts * hidden_size, inter_dim / 2});

//   auto ffn2_bias_tenosor = baidu::xpu::xft::xftVec<float>(
//         ffn1_bias ? const_cast<float*>(ffn2_bias->data<float>()): nullptr,
//         std::array<int64_t, 1>{hidden_size});
